...
interface Props {
  activeTab: 'dashboard' | 'topnews' | 'map' | 'audio';
  onTabChange: (tab: 'dashboard' | 'topnews' | 'map' | 'audio') => void;
  onScrape: () => void;
  onAnalyze: () => void;
  scraping: boolean;
  analyzing: boolean;
...
export function Header({ activeTab, onTabChange, onScrape, onAnalyze, scraping, analyzing, lastUpdated, stats }: Props) {
  // narrow type for the button click handler
  ...
  return (
    <header className="bg-[#0e0e0e] border-b border-white/5 sticky top-0 z-40">
      <div className="max-w-screen-2xl mx-auto px-5 py-0 flex items-stretch">
        {/* Logo */}
        <div className="flex items-center gap-3 pr-6 border-r border-white/5 py-3">
          ...
          <div className="flex items-center gap-1.5 ml-1">
            ...
            <span className="text-[9px] text-[#555] font-semibold tracking-widest">LIVE</span>
          </div>
        </div>

        {/* Tabs */}
        <nav className="flex items-stretch gap-0 px-2">
          {([
            { id: 'dashboard', label: 'Dashboard',          icon: '📰' },
            { id: 'topnews',   label: 'Top nieuws vandaag', icon: '🏆' },
            { id: 'map',       label: '3D Kaart',           icon: '🗺️' },
            { id: 'audio',     label: 'Audio Monitor',      icon: '🎙️' },
          ] as const).map(tab => (
            <button key={tab.id} onClick={() => onTabChange(tab.id)}
              className={`flex items-center gap-1.5 px-4 text-xs font-semibold border-b-2 transition-colors h-full ${
                activeTab === tab.id
        ...
        {/* Quick stats */}
        <div className="flex items-center gap-4 border-r border-white/5 pr-5 mr-4 hidden lg:flex">
        ...
        <div className="flex items-center gap-1.5 py-2">
          ...
          <button onClick={handleLogout} title="Uitloggen"
            className="px-2.5 py-1.5 text-[11px] rounded-lg border border-white/6 bg-[#1a1a1a] text-[#333] hover:text-[#666] transition-colors ml-1">
            ⏻
          </button>
        </div>
      </div>
